...
\end{equation}
where $\bm{H}^*$ represents the dual matrix at the optimal Lagrange multipliers $(\bm{\lambda}^*,\bm{\rho}^*)$.\footnote{We replace $\bm{H}(\bm{\lambda},\bm{\rho})$ with $\bm{H}$ when it is clear from the context.} In turn, the relaxation is tight if and only if $\mbox{corank}(\bm{H}^*)=1$. Empirically, we can study tightness by considering the largest eigenvalues of $\bm{Z}^*$ or by considering the smallest eigenvalues of $\bm{H}^*$.
}
...
\rev{NOT CERTAIN THAT WE CAN ACTUALLY DO THIS NEXT THING. IF NOT, STORY NEEDS TO CHANGE.}
...
This problem is a QCQP: $\bm{e}_{ik}$ is \emph{linear} in $\bm{C}_i$ and $\bm{t}_i$, and the constraints are quadratic. 
...
\subsubsection{Introducing Relative-Pose Measurements}\label{sec:LocRelPose}

We note that it is quite simple to introduce \textit{relative-pose measurements} of the form described in Section \ref{sec:RelPoseMeas} to Problem \eqref{opt:Localize} by simply adding the associated cost factors to the objective. Each edge in $\EdgeSet_p$ represents a cost element of the following form:
\begin{equation}
...
As a result, we found that the following set of redundant constraints could be used to tighten Problem \eqref{opt:Localize}:
...
\rev{MOVE RELATIVE POSE ANALYSES DOWN HERE}
