...
\rev{\subsection{Application to Wahba's Problem}

To make the Lemma in the preceding section more concrete,  we show its application to Wahba's Problem, Problem~\eqref{opt:Localize}. The equivalent unconstrained parameterization of this problem is the Lie-algebra vector space parameterization (CITE).
...
\begin{equation}
	...
	\begin{array}{rl}
		...
		\mbox{where} & \bm{e}_{ik} = \tilde{\bm{m}}_i^{ki} - \exp(\bm{x}_{i,r}^\wedge)\bar{\bm{C}}_i\bm{m}_0^{k0} + \bm{x}_{i,t} + \bar{\bm{t}}_i,
	\end{array}
\end{equation}
...
\subsection{Connection to Relaxation Tightness}
}
Lemma~\ref{lem:FisherInfo} can be used to relate the minimum eigenvalues of the certificate matrix and the FIM:
\begin{lemma}\label{lem:eig_bounds}
	Assume that the setting of Lemma~\ref{lem:FisherInfo} applies. Then we have the following relation:
	\begin{equation}\label{eqn:eig_bound}
		\sigma_{\min}(\bar{\bm{H}})  \leq \frac{\sigma_{\min}(\bm{\Sigma}^{-1})}{\sigma_{\min}(\bm{L})^2}, 
	\end{equation}
	where $\sigma_{\min}(\cdot)$ denotes the smallest singular value (or eigenvalue for PSD matrices \rev{, $\bar{\bm{H}}$ and $\bm{\Sigma}^{-1}$}) and $\bar{\bm{H}}$ is the certificate matrix with row and column corresponding to the homogenization variable removed.
\end{lemma}
The proof of this proposition is given in Appendix \ref{App:lem2Proof}. 
\rev{The eigenvalue bound in \eqref{eqn:eig_bound} can be seen Figure \ref{fig:cert_eig_study}.

This relationship between eigenvalues can be used to establish the following Theorem, which links the FIM and tightness of the SDP relaxation:
\begin{theorem}\label{thm:tight_fim}
Assume that the setting of Lemma~\ref{lem:FisherInfo} applies and, in addition, that strict complementarity holds for the SDP relaxation, \eqref{opt:SDP}. Then the relaxation is not tight whenever the FIM, $\Sigma^{-1}$, is degenerate (i.e. has a nullspace).
\end{theorem}

\begin{remark}[Strict Complementarity]
	The assumption of strict complementarity is required to make exact claims on the tightness of the SDP solution. Strict complementarity is a property of SDPs that has been shown to hold \emph{generically}~\cite{alizadehComplementarityNondegeneracySemidefinite1997}. However, this property has been shown to fail in highly SDPs, such as relaxations of some combinatorial problems\cite{cifuentesLocalStabilitySemidefinite2022,silvaStrictComplementarityMaxCut2018}. Nevertheless, we have empirically observed that this property holds for the problems we consider herein.
\end{remark}

Degeneracy of the FIM is directly connected to high posterior uncertainty along particular dimensions of state-estimation problems \cite{zhangDegeneracyOptimizationbasedState2016}. Therefore, Theorem \ref{thm:tight_fim} establishes a connection between posterior uncertainty and relaxation tightness. 

In practice, both tightness and degeneracy are continuous rather than discrete measures. We will see in next section that the tightness of the semidefinite relaxation degrades as the uncertainty of the optimal state-estimate increases. In particular, the introduction of anisotropic noise can cause accentuated uncertainty in a particular direction and erode the tightness of the semidefinite relaxation.
}
